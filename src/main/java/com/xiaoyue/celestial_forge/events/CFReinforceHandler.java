...
public class CFReinforceHandler {
    ...
    public static void onTooltip(ItemTooltipEvent event) {
        CFFlags.DATA_MAP.values().stream().map(IReinforce::mate).forEach(mate -> {
            if (mate == null) return;
            if (mate.test(event.getItemStack())) {
    ...
    public static void onAnvilUpdate(AnvilUpdateEvent event) {
        CFFlags.DATA_MAP.values().forEach(data -> data.onAnvilUpdate(event));
    }
