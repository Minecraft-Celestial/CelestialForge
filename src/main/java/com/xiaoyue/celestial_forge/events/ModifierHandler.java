...
public class ModifierHandler {
	...
	public static void onLivingChanged(LivingEquipmentChangeEvent event) {
		...
		ModifierHolder fromMod = ModifierUtils.getModifier(from);
		if (fromMod != null) {
			ModifierUtils.removeEquipmentModifier(entity, fromMod, slot);
		}
		ModifierHolder toMod = ModifierUtils.getModifier(to);
		if (toMod == null) {
	...
	public static void onCurioChanged(CurioChangeEvent event) {
		ItemStack from = event.getFrom();
		ItemStack to = event.getTo();
		ModifierHolder fromModifier = ModifierUtils.getModifier(from);
		...
		ModifierHolder toModifier = ModifierUtils.getModifier(to);
		if (toModifier == null) {
	...
	public static void addTooltip(ItemTooltipEvent event) {
		...
		if (ModifierUtils.canHaveModifiers(stack)) {
			ModifierHolder modifier = ModifierUtils.getModifier(stack);
			if (modifier != null) {
